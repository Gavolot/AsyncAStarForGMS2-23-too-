...
void* calculateAStar(void* input){
	...
	int iterations = 0;
	while(!current->pos.isClosestGridPositionTo(desiredPath.end, gridSize) && iterations < maxIterations){
	...
	std::cout << "StartPos: " << "x:" << desiredPath.start.x << " y:" << desiredPath.start.y << endl;
	...
	for(; current->pos != desiredPath.start; index++){
		std::cout << "index: " << index << " x:" << current->pos.x << " y:" << current->pos.y << endl;
		returnMap
	...
	//index++;
	//returnMap.addDouble("position" + to_string(index) + "valueX", desiredPath.start.x).addDouble("position" + to_string(index) + "valueY", desiredPath.start.y);
